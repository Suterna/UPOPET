...
function abrirListadoVeterinarios() {
    $("form:not('#frmListadoVeterinarios')").parent("fieldset").hide("normal");
    $("#tabla").addClass("oculta");
    // Oculto todos los formularios menos este
    $("container:not('#listadoVeterinarios')").parent("div").hide("normal");

    // Verifico si ya he cargado el formulario antes
    if ($('#frmListadoVeterinarios').length == 0) {
        $("<div>").appendTo('#formularios').load("./paginas/listados/listadoVeterinarios/listadoVeterinarios.html",
            function() {
                $.getScript("./paginas/listados/listadoVeterinarios/listadoVeterinarios.js");
            });

    } else {
        // Lo muestro si está oculto
        $('#frmListadoVeterinarios').parent().show("normal");
        $('#listadoVeterinarios').show("normal");
    }
}

function abrirListadoClientes() {
    $("form").parent("fieldset").hide("normal");
...
function abrirListadoMascotas() {
    // Oculto todos los formularios menos este
    $("form:not('#frmListadoMascotas')").parent("fieldset").hide("normal");
    $("#tabla").addClass("oculta");
    // Verifico si ya he cargado el formulario antes
    if ($('#frmListadoMascotas').length == 0) {
        $("<div>").appendTo('#formularios').load("./paginas/listados/listadoMascotas/listadoMascotas.html",
            function() {
                $.getScript("./paginas/listados/listadoMascotas/listadoMascotas.js");
            });

    } else {
        // Lo muestro si está oculto
        $('#frmListadoMascotas').parent().show("normal");
